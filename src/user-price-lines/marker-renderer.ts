...
export class MovableMarkerPaneRenderer implements ISeriesPrimitivePaneRenderer {
    private _data: MarkerDataItem[] = [];
    // Store icon positions
    private _iconBounds: { x: number; y: number; width: number; height: number; id: string }[] = [];

    update(data: MarkerDataItem[]) {
        this._data = data;
        this._iconBounds = []; // Clear previous bounds
    }
    ...
    draw(target: CanvasRenderingTarget2D): void {
        ...
        target.useMediaCoordinateSpace(scope => {
            ...
            const ctx = scope.context;
                ...
                ctx.fillText(`${priceY} x ${100}`, markerX + markerWidth / 2, markerY + markerHeight / 2);
